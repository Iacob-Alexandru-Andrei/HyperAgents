...
class MetaAgent(AgentSystem):
    ...
    def forward(self, repo_path, eval_path, iterations_left=None):
        ...
        instruction = (
            ...
            f"`{repo_path}lineage/MODEL_CATALOG.md`. Use the `query_model` tool "
            "to fire a one-shot call to a different model with "
            "`(prompt, id, effort, max_tokens)` -- useful for offloading bulk "
            "text generation to a cheaper model, or sanity-checking your "
            "reasoning against a different model. The call has no chat "
            "history and no tools; it's tokens in, tokens out."
            f"\n\nLive remaining budget for this expansion is prepended to "
            "every chat turn by the host harness."
            ...
            "reasoning clearly."
            "\n  - `agent_output/model_patch.diff` -- the code edit the "
            "predecessor's meta-agent produced. Absent on `gen_initial/` "
            "(no agent edits there)."
